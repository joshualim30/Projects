...
let cachedBackgroundText: string | null = null;
...
const getBackgroundContent = async (): Promise<string> => {
    if (cachedBackgroundText) return cachedBackgroundText;
...
export const chatWithGemini = functions.runWith({
    maxInstances: 10,
    memory: '512MB'
    ...
    console.log(`Resume loaded: ${resumeText?.length || 0} characters`);
    console.log(`Background loaded: ${backgroundText?.length || 0} characters`);
